...
exports.Service = class Service {
    ...
    constructor(server, options) {
        ...
        if (typeof this.teardown === 'function') {
            this.server.ext('onPreStop', this.teardown, { bind: this });
        }
    ...
    caching(options) {
        ...
        Object.keys(options).forEach((methodName) => {
            ...
            const cache = options[methodName].cache ?
                Hoek.shallow(options[methodName].cache) :
                Hoek.shallow(options[methodName]);

            delete cache.generateKey;

            this.server.methods(
                `schmervices.${this.constructor.name}.${methodName}`,
                this[methodName],
                {
                    bind: this,
                    generateKey,
            ...
            this[methodName] = this.server.methods.schmervices[this.constructor.name][methodName];
        });
    }
};
...
exports.plugin = {
    ...
    register(server, options) {
        ...
        server.decorate('request', 'services', internals.services((request) => request.route && request.route.realm));
